...
def wallpaper():
    ...
    w += 1
    if w < len(wall_list):
        wall_wid.configure(image=wall_list[w])
    else:
        w = 0
        wall_wid.configure(image=wall_list[w])
def destroy_widgets():
...
wall1 = PhotoImage(file="images\\wall1.png")
wall2 = PhotoImage(file="images\\wall2.png")
wall3 = PhotoImage(file="images\\wall3.png")
wall4 = PhotoImage(file="images\\wall4.png")
wall_list = [wall1, wall2, wall3, wall4]
wall_wid = Label(window, image=wall_list[w])
w_but = Button(window, text="BG", command=wallpaper)
wall_wid.place(x=-10, y=0)
w_but.place(x=350, y=550)
